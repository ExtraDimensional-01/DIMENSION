...
export async function generateLicensePdf(params: LicensePdfParams): Promise<Uint8Array> {
  ...
  if (snapshot.distributionAllowed) {
    y -= 6;
    drawParagraph(
      `Distribution: Allowed on streaming, video, and digital platforms, including SoundCloud, YouTube, YouTube Music, Spotify, Apple Music, Amazon Music, TIDAL, Deezer, and other comparable services, subject to the applicable platform's rules and the limitations of this license. This permission applies to the Licensee's finished song incorporating the licensed instrumental — not to the instrumental itself, which remains owned by the Licensor. Uploading the finished song to platforms such as SoundCloud or YouTube is part of normal distribution under this permission and does not require Social Media / Content Use to also be enabled. Any stream, sales/copy, commercial-use, or other limitation set above still applies regardless of which platform the finished song is distributed through. DIMENSION does not guarantee acceptance or approval by any platform; the Licensee is responsible for complying with each platform's own rules and requirements.`,
